...
# Google Fit Scopes - Modify if you need additional permissions
...
def authenticate_google_fit(client_secrets_file):
    ...
    try:
        flow = Flow.from_client_secrets_file(
            client_secrets_file,
            scopes=SCOPES,
            redirect_uri="https://smart-watch-integration.streamlit.app/"  # Make sure this matches Google Cloud Console
        )
        auth_url, _ = flow.authorization_url(prompt='consent', access_type="offline")
        return auth_url, flow
    except Exception as e:
        raise Exception(f"Failed to generate authentication URL: {e}")
...
def fetch_google_fit_data(credentials_json):
    ...
    try:
        ...
        raise Exception(f"Failed to fetch Google Fit data: {e}")
